...
在开始深入这些方面之前，我们需要讨论一下术语 **字符串** 的具体意义。Rust 的核心语言中只有一种字符串类型：字符串 slice（`str`），它通常以被借用的形式（`&str`）出现。第四章中我们讲到了 **字符串 slice**：它们是一些储存在别处的、使用 UTF-8 编码的字符串数据的引用。例如，字符串字面值被储存在程序的二进制文件中，因此是字符串 slice 类型。
